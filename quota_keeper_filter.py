    ...
    if isinstance(pri, dict):
        ...
        if ov is not None and not isinstance(ov, dict):
            ...
            for k, v in ov.items():
                ...
                # legacy-direct / multiplier-only), not just alias (v0.4.17)
...
def qk_find_pricing(model_id: str, table: dict, overrides: Optional[dict] = None):
    ...
    `multiplier` is INDEPENDENT of `alias` (v0.4.17): it scales a manual
    ...
    async def stream(
        ...
        try:
            ...
            if isinstance(ev, str):
                if '"usage"' not in ev and '"usageMetadata"' not in ev and '"usage_metadata"' not in ev:
    ...
    async def outlet(
        ...
        try:
            if not isinstance(body, dict):
                return body
            tok = qk_normalize_usage(qk_extract_usage_obj(body))
            if tok is None:
