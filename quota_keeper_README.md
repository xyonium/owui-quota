...
- `schedule.timezone`（默认 `Asia/Shanghai`）决定账本的日界（日/月配额周期按哪个时区切天）和 TOU 时间窗的兜底时区（TOU 另有自己的 timezone 可覆盖）。
- **v0.4.0 移除了夜间/周末配额倍数**（`night_multiplier`/`weekend_multiplier`）：那种"闸门高度随小时变化"的配额在半衰点会把已超支用户直接断供，语义也与 TOU 方向相反，弊大于利。config.json 里遗留的这些键会被忽略；要做分时段经济杠杆请用下面的 TOU（乘的是价格而不是配额上限，行为正常）。
