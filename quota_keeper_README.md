...
| 页面只有标题、正文空白；网络面板里部分 `/api/v1/quota-keeper/*` 返回 9 字节纯文本 `403 Forbidden` | **前置网关/WAF 限流**（非插件问题）：实测该域名突发 8 连请求约一半被 403、每秒 1 次也偶发；页面加载是 4 路并发，任一被拦旧版会整页空白 | 在网关（WAF/反代）上放松对该域名的限流；>= 0.2.3 起单个接口失败只弹 toast 提示，其余区块照常渲染 |
