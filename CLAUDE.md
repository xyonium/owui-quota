...
`quota_keeper_README.md` (user-facing install/config guide, Chinese) and `docs/quota_keeper_HANDOFF.md` (detailed dev handoff with algorithms, known limitations, and roadmap — **read it before non-trivial changes**; note it's a v0.2.0-era snapshot archived under `docs/`, so a few descriptions like the removed night/weekend quota multipliers reflect the old design, not current code) are the authoritative docs.
...
- Logic can be unit-tested without Open WebUI by stubbing pydantic (`sys.modules['pydantic']` injection); regression cases for usage normalization, price matching, quota resolution, and TOU rates are listed in `docs/quota_keeper_HANDOFF.md` §7.
...
- **Quota resolution priority**: per-user quota > max of user's group quotas > default > unlimited. Usage is summed over `quota_period` (daily/weekly/monthly), day-bucketed in `schedule.timezone`. (The night/weekend quota multipliers were removed in v0.4.0 — `qk_time_multiplier` always returns 1.0; use TOU for time-based economics.)
- **Cost model**: `credits_per_usd` (default 1000 credits = $1). Prices come from an upstream URL (LiteLLM flat format or models.dev nested format, auto-detected), cached per-1M-token. TOU (`config.tou`, see HANDOFF §5.7) multiplies the whole per-request cost by the active tier's rate (peak/offpeak/normal windows + holidays; resolution models → providers → `default_policy`), orthogonal to the quota ceiling; ledger records per-tier request counts and `cost_saved_usd`.
- **Price matching** (`qk_find_pricing`): override → exact → date-suffix-stripped exact → path suffix → path segment → longest substring (`contains`), with `.`↔`-` normalization. Override values accept three shapes: `{prices:{...}}` (or a bare legacy price dict), `{alias:"<key>", multiplier:m}` (resolves the target through the same chain incl. nested overrides, cycle-safe ≤8 hops, scales per-1M prices), and `null` (clears the override). Unmatched models cost 0 and get flagged `priced=false` in the ledger (per-request `unpriced_requests` counter drives the flag).
- **API** (all require a signed-in session; admin-only routes enforce `role=admin`, the rest self-scope to the caller's own data for non-admins): `GET/POST /config` (validate + deep-merge), `GET /users /groups /ledger` (admin), `GET /pricing` (admin; summary, `?full=1` for the editor), `GET /recent` (self-scoped), `GET /stats?from&to&user&model&granularity=hour|day` (self-scoped; server-side aggregation: KPI, stacked series, per-user/per-model rows), `POST /pricing/refresh`, `POST /pricing/reprice`, `GET /pricing/match` (admin), `GET /models` (self-scoped), `GET /me` + `GET /me/usage` (own-data-only) back the non-admin card. Failures raise `HTTPException` (401/403) from the auth dependencies; routes are spliced ahead of OWUI's `spa-static-files` catch-all mount by `_mount_guard` (plain `include_router` appends land *after* it and are shadowed — v0.2.1 fix, prune pattern), which also drops stale same-prefix routes so hot code updates take effect without restart; the pricing loop task is tracked on `app.state` (cancelled and respawned on remount) and runs via `asyncio.create_task` + `asyncio.to_thread`.
...
- **Dedup**: three layers — usage is recorded once per response id (`_seen` OrderedDict, capped 4096); the 0.11 stream-end outlet tops up via `_seen_msgids` (webui chats carry `__metadata__.message_id`); API clients send no message id, so the stream-end outlet echo (0.11.1 runs outlet inline for temp/API chats with a freshly generated body id) is matched by content against recent `stream()` records (`_stream_rec`, same user + field-wise token totals within 30s) and merged as a zero-delta topup. Orphan adoption in outlet is unconditional when the user is present.
...
- Routes under a *changed* `route_prefix`/`api_prefix` valve linger until restart (stale cleanup can only find current-prefix paths) — valve descriptions say so (HANDOFF §8.2 residual / §8.15).
