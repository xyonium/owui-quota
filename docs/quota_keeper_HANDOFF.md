...
> **归档说明**：本文档是 v0.2.0（2026-08）的设计/交接快照，归档于 `docs/` 作为**历史设计文档**保留。其中部分描述（如夜间/周末配额倍数、配额周期、"未经生产验证"等早期状态）反映的是当时版本，**可能与当前代码不一致**；以仓库根目录的两份 README、`CLAUDE.md` 及代码本身为准。文档内容按原样保留，未逐条订正。
